...
class PostsApiImpl : PostsApi {
    ...
    override fun fetchPosts(withUpdates: Boolean): Single<List<Post>> =
        Single.fromCallable {
            if (withUpdates) {
                updatedPosts
            } else {
    ...
    private fun parseJsonWithPosts(withUpdates: Boolean = false): List<Post> =
        PostUtils.parsePosts(
            readJson(
                getJsonFile(withUpdates)
            )
        )
